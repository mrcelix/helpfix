          ...
          {DAY_ORDER.map((day) => {
            ...
            return (
              <div key={day} className="flex items-center gap-2.5 bg-[var(--panel-2)] border border-[var(--border)] rounded-lg px-3 py-2">
                <button
                  onClick={() => (isOpen ? closeDay.mutate(day) : setDay.mutate({ dayOfWeek: day, startTime: '09:00', endTime: '18:00' }))}
                ...
                {isOpen ? (
                  <div className="flex items-center gap-1.5 flex-1">
                    <input
                      ...
                      onChange={(e) => setDay.mutate({ dayOfWeek: day, startTime: e.target.value, endTime: row.end_time.slice(0, 5) })}
                    ...
                    <input
                      ...
                      onChange={(e) => setDay.mutate({ dayOfWeek: day, startTime: row.start_time.slice(0, 5), endTime: e.target.value })}
                      className="bg-[var(--panel)] border border-[var(--border)] rounded-md px-2 py-1 text-[12px]"
                    />
                  </div>
                ) : (
